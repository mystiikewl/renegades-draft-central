...
import { Search, X, Filter, ChevronDown, Mail } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
interface UserSearchFilterProps {
  users: Profile[];
  teams: Team[];
  onFilteredUsers: (filteredUsers: Profile[]) => void;
  className?: string;
  placeholder?: string;
  showFilters?: boolean;
}
...
/**
 * Search and filter component for users
 *
 * Features:
 * - Real-time search with debouncing
 * - Filter by team assignment (assigned to team, no team)
 * - Clear filters functionality
 * - Mobile-optimized interface
 * - Visual feedback for active filters
 */
export const UserSearchFilter = React.memo(({
  users,
  teams,
  onFilteredUsers,
  className,
  ...
  // Filter options
  const filterOptions = [
    { id: 'has-team', label: 'Has Team', filter: (user: Profile) => !!user.team_id },
    { id: 'no-team', label: 'No Team', filter: (user: Profile) => !user.team_id },
  ];
  ...
  const filteredUsers = useMemo(() => {
    ...
    return filtered;
  }, [users, debouncedSearchTerm, activeFilters]);

  // Update parent with filtered users
  React.useEffect(() => {
  ...
  const getUserTeamName = (user: Profile) => {
