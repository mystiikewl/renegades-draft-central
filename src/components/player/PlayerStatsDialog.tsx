...
function PlayerProfileBody({
  ...
  Pick<PlayerProfileDialogProps, 'canPick' | 'picking' | 'pickNumber' | 'onPick'> & {
  ...
  const draftAction = canPick ? (
    <Button
      size="sm"
      className="rounded-full px-5 font-bold transition-transform active:scale-[0.98]"
      disabled={picking}
  ...
  return (
    <>
      <div className="flex max-h-[inherit] flex-col overflow-y-auto bg-background">
        <div className="relative min-h-44 shrink-0 overflow-hidden border-b bg-gradient-to-br from-muted via-background to-muted/40 px-4 pb-4 pt-5 pr-12 sm:min-h-48 sm:px-6 sm:pt-6 sm:pr-14">
          ...
          <div className="absolute bottom-0 right-2 sm:right-8">
            <PlayerHeadshot espnId={player.espn_id} name={player.name} size={142} variant="bare" className="sm:hidden" />
            <PlayerHeadshot espnId={player.espn_id} name={player.name} size={168} variant="bare" className="hidden sm:inline-block" />
          </div>

          <div className="relative z-10 max-w-[62%] sm:max-w-[58%]">
            <h2 className="line-clamp-2 text-xl font-black uppercase leading-tight tracking-tight sm:text-2xl">{player.name}</h2>
            ...
            {canPick && <div className="mt-4 sm:hidden">{draftAction}</div>}
          </div>
        </div>
        ...
        <div className="border-b bg-card px-4 py-3 sm:px-6">
          <div className="flex items-center justify-between gap-4">
            ...
            {canPick && <div className="hidden sm:block">{draftAction}</div>}
          </div>
        </div>
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Draft {player.name}?</AlertDialogTitle>
            <AlertDialogDescription>
              {pickNumber ? `This will submit ${player.name} as pick #${pickNumber}. ` : ''}
              If the draft moves before you confirm, the server will reject the stale selection rather than apply it to a later pick.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={picking}>Keep scouting</AlertDialogCancel>
            <AlertDialogAction
              disabled={picking}
              onClick={(event) => {
            ...
            >
              {picking ? 'Submitting…' : 'Confirm pick'}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
...
const LOG_COLS: { key: keyof GameLogRow; label: string }[] = [
  { key: 'date', label: 'Date' }, { key: 'opponent', label: 'Opp' }, { key: 'min', label: 'MIN' },
        ...
        isLoading ? (
          ...
          <div className="max-h-80 overflow-auto border-t">
            <table className="w-full min-w-[34rem] text-xs">
              <thead className="sticky top-0 bg-card shadow-[0_1px_0_0_var(--border)]"><tr className="text-[10px] uppercase tracking-wide text-muted-foreground">
                {LOG_COLS.map(({ key, label }) => <th key={key} className={`whitespace-nowrap px-3 py-2 font-bold ${key !== 'date' && key !== 'opponent' ? 'text-right' : 'text-left'}`}>{label}</th>)}
              </tr></thead>
              <tbody>
                {rows.map((r, index) => (
                  <tr key={r.gameId} className={`border-t border-border/40 ${index % 2 ? 'bg-muted/[0.18]' : ''}`}>
                    ...
                    <td className="whitespace-nowrap px-3 py-2 text-muted-foreground">
                      ...
                      <span className={r.result === 'W' ? 'font-semibold text-emerald-600 dark:text-emerald-400' : r.result === 'L' ? 'text-red-500 dark:text-red-400' : ''}>{r.result}</span>
                    </td>
                    {LOG_COLS.slice(2).map(({ key }) => <td key={key} className="whitespace-nowrap px-3 py-2 text-right tabular-nums">{r[key]}</td>)}
                  </tr>
                ))}
              </tbody>
            </table>
